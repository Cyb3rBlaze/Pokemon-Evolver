...
class VAE(nn.Module):
    def __init__(self, config, device=torch.device("cpu")):
        ...
        # input: batch_size x 512 x 4 x 4, output: batch_size x 512 x 2 x 2
        self.encoder_blocks += [EncoderBlock(config.encoder_in_channels*16, config.encoder_in_channels*16, device)]
        ...
        # input: batch_size x 512 x 2 x 2, output: batch_size x 512 x 4 x 4
        self.decoder_blocks = [DecoderBlock(int(config.decoder_in_channels), int(config.decoder_in_channels), device)]
        # input: batch_size x 512 x 4 x 4, output: batch_size x 256 x 8 x 8
        self.decoder_blocks += [DecoderBlock(int(config.decoder_in_channels), int(config.decoder_in_channels/2), device)]
        ...
        # input: batch_size x 32 x 64 x 64, output: batch_size x 3 x 128 x 128
